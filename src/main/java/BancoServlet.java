...
public class BancoServlet extends HttpServlet {
    ...
    private String recuperarNomeCliente(HttpServletRequest request) {
    	...
    	String nome = new BancoBusiness().recuperarNomeCliente(contaBancaria);
    	return criarRetornoOperacaoBancaria(nome);
    }
	...
	private Map<Integer, ContaBancaria> recuperarContasDaSessaoDoUsuario(HttpServletRequest request) {
    	...
    	if(request.getSession().getAttribute(ATRIB_CONTAS).equals(null)) {
    		request.getSession().setAttribute(ATRIB_CONTAS, contas);
    		return contas;
    	}else {
    		contas = (Map<Integer, ContaBancaria>)request.getSession().getAttribute(ATRIB_CONTAS);
        	return contas;
    	}
    }
