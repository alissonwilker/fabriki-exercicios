...
public class BancoServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(operacao == "recuperarCpfCliente"){
			out.println("<div id='resposta'>" +  cpfCnpj + "</div>");					

		}

		if(operacao == "recuperarCnpjCliente"){
			out.println("<div id='resposta'>" + cpfCnpj + "</div>");					

		}

		if(operacao == "recuperarSaldo"){
			out.println("<div id='resposta'>" + cb.getSaldo() + "</div>" );

		}

		if(operacao == "depositarValor"){
			cb.depositar(valorDeposito);
			out.println("<div id='resposta'>" + cb.getSaldo()  + "</div>");

		}

		if(operacao == "sacarValor"){
			cb.sacar(valorSaque);
			out.println("<div id='resposta'>" + cb.getSaldo() + "</div>");

		}

		if(operacao == "depositarSacarValor"){
			cb.depositar(valorDeposito);
			cb.sacar(valorSaque);
			out.println("<div id='resposta'>" + cb.getSaldo() + "</div>");
		}
