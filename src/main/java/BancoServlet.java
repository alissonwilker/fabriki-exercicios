...
public class BancoServlet extends HttpServlet {
    ...
    private Map<Integer, ContaBancaria> recuperarContasDaSessaoDoUsuario(HttpServletRequest request) {
        ...
        String contaBancaria = (String)session.getAttribute(PARAM_ID_CONTA);
        session.setAttribute(PARAM_ID_CONTA, nomeCliente);       
        if(!session.isNew()) {
        	session.invalidate();
        }

        Map<Integer, ContaBancaria> contasBancarias = recuperarContasDaSessaoDoUsuario(request);
        return (Map<Integer, ContaBancaria>) contasBancarias.get(idConta);
    }
