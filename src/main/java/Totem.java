...
public class Totem {
    ...
    /**
     * Gera e adiciona no final da estrutura de dados correspondente uma nova
     * senha de atendimento que pode ser prioritÃ¡ria se o valor de prioridade
     * for
     * <b>true</b>, ou nÃ£o prioritÃ¡ria se o valor de prioridade for
     * <b>false</b>.
     *
    ...
    public Senha gerarSenha(boolean prioridade) {
        Senha senhaGerada = null;
        if(prioridade == true){
            senhaGerada = gerarSenhaPrioridade();
            filaPrioridade.add(senhaGerada);
        }else{
            senhaGerada = gerarSenhaNormal();
    ...
    /**
     * Chama a prÃ³xima senha para atendimento. O critÃ©rio de ordem de chamada
     * da senha se define por: enquanto a fila de prioridade tiver senhas ainda
     * nÃ£o atendidas, a senha retornada serÃ¡ da fila de prioridade. Se a fila
     * de prioridade nÃ£o tiver senhas ainda nÃ£o atendidas, ou seja, se estiver
     * vazia, a senha retornada serÃ¡ da fila normal.
     *
     * @return a senha que serÃ¡ atendida.
     */
    public Senha chamarSenha() {
        Senha senhaChamada = null;
        if(getTamanhoFilaNormal() == 0 && getTamanhoFilaPrioridade() == 0){
            return senhaChamada;
        }
        if(getTamanhoFilaPrioridade() != 0){
            senhaChamada = this.filaPrioridade.remove(0);
        }else if(getTamanhoFilaNormal() != 0){
            senhaChamada = this.filaNormal.remove(0);
        }
        return senhaChamada;
    }
    ...
    public Senha verificarProximaSenha() {
        ...
        if(getTamanhoFilaPrioridade() != 0){
            senhaChamada = this.filaPrioridade.get(0);
        }else if(getTamanhoFilaNormal() != 0){
            senhaChamada = this.filaNormal.get(0);
        }
        return senhaChamada;
    }
